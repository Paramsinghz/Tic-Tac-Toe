...
p1=""
root = Tk()
root.title('Tic-Tac-Toe game made by Paraminder Singh') # Main title of Board
check=[]

winner=False
clicked = True
count = 0 # how much button clicked
turn=0 # whose turn to play
ai=1
# Defining the Moves names
player = 'X'
bot = 'O' 

def  disable_all_buttons():  # Close game when finished
...
# Win condition of Board markings  
def winch(mark):
...
# Both Function playes after First move


# Compute the Moves Mini-Max algorithm   
def compMove():
    bscor = -800
...
def minimax(board, depth, isMaximizing):
    ...
    # Return the move in Mini Max algorithm , Which position played
def comp():
    but=compMove()
...
# Return the message after winning    
def checkifwon():
    ...
    if ai==1: 
        p2="COMPUTER WIN"
    elif ai==0:    # When play with computer
...
# When match draw message display
def draw():
...
# Click show X or O , toggle the sides of X, O
def b_click(b,num):
...
# Main Frame of the Design Board
